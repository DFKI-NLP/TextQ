...
def load_data():
    ...
    feature_cols = [col for col in features_df.columns if col not in [
        'sentence', 'sprachliche logik', 'komplexität', 
...
# Feature Processing
def select_features(train_data, feature_cols, target_cols, n_features=20):
...
def compute_metrics(eval_pred):
    ...
    accuracy = [(abs(logits[:, i] - labels[:, i]) < 0.5).mean() for i in range(4)]
...
def main():
    ...
    target_cols = ['sprachliche logik', 'komplexität', 'eindeutigkeit', 'vorhersehbarkeit']
    ...
    for fold, (train_idx, test_idx) in enumerate(splits):
        ...
        train_val_data = df.iloc[train_idx]
        ...
        # 2. Prepare BERT datasets
        def prepare_dataset(df):
            texts = df['sentence'].tolist()
            labels = df[target_cols].values.astype(float)
        ...
        y_train_val = train_val_data[target_cols].values
        ...
        # Epsilon-insensitive squared error (ε=0.25)
        # EISE calculation (ε = 0.25)
        epsilon = 0.25
        squared_error = (test_pred - y_test) ** 2
    ...
    # Final evaluation
    avg_metrics = {
        'avg_mse': np.mean([r['mse'] for r in svm_results]),
        'avg_mae': np.mean([r['mae'] for r in svm_results]),
        'avg_rmse': np.mean([r['rmse'] for r in svm_results]),
        'avg_r2': np.mean([r['r2'] for r in svm_results]),
        'avg_eise': np.mean([r['epsilon_insensitive_squared_error'] for r in svm_results])
    }
    ...
    print(f"Average Epsilon-Insensitive Squared Error: {avg_metrics['avg_eise']:.4f}")
