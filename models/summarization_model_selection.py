...
datasets = [{"train": train_df, "validation": val_df, "test": test_df} for _ in models]
...
for i in range(len(models)):
    ...
    for split in ds:
        ...
        ds[split] = ds[split].drop(columns=["sprachliche logik", "komplexität", "eindeutigkeit", "vorhersehbarkeit"])
...
def create_training_args(model_idx, fold):
    ...
    return TrainingArguments(
        output_dir="Outputs/"+ dataset_name +"/" + model_name + f"/fold_{fold}",
...
for model_idx, (tokenizer, model) in enumerate(models):
    ...
    for fold, (train_idx, test_idx) in enumerate(splits):
        ...
        def preprocess_dataset(df, tokenizer):
            ...
            df = df.drop(columns=["sprachliche logik", "komplexitÃ¤t", "eindeutigkeit", "vorhersehbarkeit"])
